...
- `/api/v1/orcflo/runs`, `/api/v1/orcflo/runs/:runId`, `/api/v1/orcflo/runs/:runId/stream` — Orcflo run engine: start/list runs and replay the run event stream (SSE); executes graphs with deterministic conditions (`condition` nodes + `condition` edges), routers (structured route selection), bounded `for_each` loops (`loop`/`loopExit` edges, `maxItems`/`maxIterations` limits), and parallel execution of independent nodes (`maxConcurrency`, default 4)
...
- `/api/v1/orcflo/interfaces`, `/api/v1/orcflo/interfaces/:slug/run` — public workflow interfaces (§34): expose a READY workflow to anonymous callers as a public form with input-schema validation, per-interface rate/daily-run/cost limits, and idempotent runs (the run endpoint needs no authentication)
