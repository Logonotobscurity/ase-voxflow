...
export function VendorDashboard() {
  ...
  function exportCsv() {
    ...
    setNotice(`Exported ${list.length} sample vendor records.`);
  }

  return <><Header /><main>
    <section className="page-hero vendor-hero"><div className="container"><p className="eyebrow"><span className="eyebrow-dot" /> Business operations demo</p><h1>Vendor command centre.</h1><p>Explore sourcing, assessment and purchase-control interfaces with scripted sample records. No live vendor system is connected.</p><div className="hero-actions"><Link className="btn btn-dark" href="/app/canvas?template=vendor-registration">Design an intake flow</Link><Link className="btn btn-light" href="/app/voice">Open voice preview</Link></div></div></section>
    ...
    <section className="section section-cream vendor-workspace"><div className="container">
      ...
      <p className="vendor-view-result">Illustrative metrics · not live business data</p><div className="kpi-grid vendor-kpis"><div className="kpi"><span>Sample active vendors</span><strong>284</strong><small>↑ 12 this month</small></div><div className="kpi"><span>On-time delivery</span><strong>94.2%</strong><small>↑ 2.8% quarter</small></div><div className="kpi"><span>Open PO value</span><strong>₦28.4m</strong><small>37 orders</small></div><div className="kpi"><span>At-risk vendors</span><strong>18</strong><small className="risk-copy">5 require action</small></div></div>
      ...
      <div className="vendor-board"><div className="vendor-toolbar"><label className="vendor-search"><Search size={14} /><span className="sr-only">Search vendor records</span><input className="field" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search vendors, categories or cities…" /></label><button aria-pressed={riskOnly} className={`btn btn-ghost ${riskOnly ? 'active-filter' : ''}`} onClick={() => setRiskOnly((value) => !value)}><Filter size={14} /> {riskOnly ? 'Showing risks' : 'Risk filter'}</button><button className="btn btn-ghost" onClick={exportCsv} aria-label="Export current vendor view"><Download size={14} /> Export</button><Link className="btn btn-dark" href="/app/canvas?template=vendor-registration"><Plus size={14} /> Add vendor</Link></div>
        <p className="vendor-view-result" aria-live="polite">{notice || `${list.length} sample records in ${tab.toLowerCase()} view`}</p>
      ...
      <Link href="/app/voice?prompt=Compare%20logistics%20vendor%20performance" className="voice-callout vendor-voice"><Mic2 size={20} /><p><strong>Try the scripted voice preview</strong>Use the sample prompt “Compare logistics vendor performance across Lagos and Accra this quarter.”</p><span>Open Voice Studio</span></Link>
    </div></section>
...
function VendorDetail({ vendor, onClose }: { vendor: Vendor; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div className="market-modal" role="dialog" aria-modal="true" aria-labelledby="vendor-dialog-title"><div className="modal-cover"><button ref={closeRef} className="modal-close" onClick={onClose} aria-label="Close vendor detail"><X size={17} /></button><div className="modal-avatar">{vendor[0].split(' ').map((word) => word[0]).join('').slice(0, 2)}</div></div><div className="modal-body"><span className="tag tag-orange">{vendor[2]} VENDOR</span><h2 id="vendor-dialog-title">{vendor[0]}</h2><p className="muted vendor-detail-meta">{vendor[1]} · {vendor[5]} · Illustrative record</p><div className="stats-ribbon vendor-detail-stats"><div className="stat"><strong>{vendor[3]}</strong><span>Performance</span></div><div className="stat"><strong>₦15m</strong><span>Credit limit</span></div><div className="stat"><strong>Low</strong><span>Risk level</span></div></div><div className="modal-features"><div className="modal-feature"><CheckCircle2 size={14} /> Sample compliance state</div><div className="modal-feature"><ShieldCheck size={14} /> Sample risk state</div><div className="modal-feature"><TrendingUp size={14} /> Sample performance trend</div><div className="modal-feature"><ShoppingCart size={14} /> Sample order count</div></div><div className="modal-actions"><Link className="btn btn-dark" href={`/app/canvas?template=purchase-order&vendor=${encodeURIComponent(vendor[0])}`}><ShoppingCart size={14} /> Draft PO flow</Link><Link className="btn btn-light" href="/resources">View documents</Link></div></div></div></div>;
}
