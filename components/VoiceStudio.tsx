...
export function VoiceStudio() {
  ...
  return <><Header /><main>
    <section className="page-hero voice-studio-hero"><div className="container"><p className="eyebrow"><span className="eyebrow-dot" /> Agent voice studio</p><h1>Give your agent a visual personality.</h1><p>Design and test realtime voice experiences with responsive visualizers, transcript blocks and production-ready media controls.</p><div className="hero-actions"><Link className="btn btn-dark" href="#designer">Design a visualizer</Link><Link className="btn btn-light" href="/app/canvas?template=voice-agent">Add to a workflow <ArrowRight size={15} /></Link></div></div></section>
    ...
    <section className="section section-cream" id="designer"><div className="container"><div className="section-head"><div><p className="eyebrow"><span className="eyebrow-dot" /> Visual state designer</p><h2 className="h2">Make every agent state legible.</h2></div><p className="lede">Choose a surface, state and signal colour. The preview updates immediately and remains understandable beyond animation alone.</p></div><div className="personality voice-designer">
      <div className="voice-controls"><fieldset><legend className="panel-label">Visualizer style</legend><div className="style-pills">{styles.map((item) => <button aria-pressed={visualStyle === item} className={`style-pill ${visualStyle === item ? 'active' : ''}`} key={item} onClick={() => setVisualStyle(item)}>{item}</button>)}</div></fieldset>
        ...
        <div className="voice-callout"><AudioLines size={20} /><p><strong>Component-ready</strong>Every style maps to WebRTC and agent state events without relying on colour alone.</p></div>
      </div>
    ...
    <section className="section section-ink"><div className="container"><div className="section-head"><div><p className="eyebrow eyebrow-light"><span className="eyebrow-dot" /> Session simulator</p><h2 className="h2">A complete agent exchange.</h2></div><p className="lede">Test transcript hierarchy, audio feedback and media controls in one accessible surface.</p></div><div className="voice-console voice-session"><div className="console-top"><div className="ai-top"><span className="ai-icon"><Mic2 size={14} /></span>Ayo · Vendor assistant</div><span className={`tag ${live ? 'tag-green' : 'tag-orange'}`}>{live ? 'WebRTC connected' : 'Session paused'}</span></div><div className="spectrum voice-session-spectrum">{bars.map((height, index) => <span key={index} style={{'--h': `${live ? height : 8}px`, '--i': index} as React.CSSProperties} />)}</div><div className="voice-transcript"><div className="msg">Hi, how can I help with vendor operations today?</div><div className="msg user">Show my preferred logistics vendors in Lagos.</div><div className="msg">I found 12 active vendors. TransWest Africa has the highest current score at 96%.</div></div><div className="console-actions voice-media-controls"><button aria-pressed={camera} className={`btn btn-icon btn-outline-light ${camera ? 'is-on' : ''}`} onClick={() => setCamera((value) => !value)} aria-label={camera ? 'Turn camera off' : 'Turn camera on'}><Video size={16} /></button><button className="mic-orb" onClick={() => setLive((value) => !value)} aria-label={live ? 'Pause microphone' : 'Resume microphone'}>{live ? <Mic2 size={22} /> : <Play size={22} />}</button><button className="btn btn-icon btn-outline-light" onClick={() => setLive(false)} aria-label="End voice session"><PhoneOff size={16} /></button></div></div></div></section>
  </main><Footer /></>;
}
