...
| Orcflo run engine | Runs with a replayable event stream, opt-in content-hashed step cache, metering, fail-closed model gateways, four trigger kinds, and blueprint instantiation into first-class workflows. Shares the canonical workflow contract, event/outbox envelope, and unit-of-work boundary. | Unit-, route-, and live-HTTP-tested in demo/memory mode; migration SQL verified on PostgreSQL 16 (PGlite WASM); Prisma adapter server path ready but `UNVERIFIED` until the next disposable-cluster run. |
