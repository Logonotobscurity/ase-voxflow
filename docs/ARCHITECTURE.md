...
| Implement deterministic branch evaluation and connect `agent` workflow nodes. | Branch coverage, persisted decisions, bounded-runtime wiring, and observable evidence. | **Workflow Runtime Owner** | PARTIALLY RESOLVED (2026-08-16): `agent` workflow nodes execute through the canonical `BoundedAgentRuntime`; deterministic per-edge conditional selection (`condition` nodes + `condition` edges), router nodes, bounded `for_each` loops (loop-aware graph validation), and wave-based parallel execution (`maxConcurrency`) are implemented in the Orcflo engine (control-flow + parallel increments). Remaining: branch coverage metrics and persisted decision records as a first-class entity. |
...
| Orcflo run engine | Runs with a replayable event stream, opt-in content-hashed step cache, metering, fail-closed model gateways (`ai_model` nodes), four trigger kinds, blueprint instantiation, deterministic control flow (`condition` nodes/edges, `router` nodes, bounded `for_each` loops with `loop`/`loopExit` edges and loop-aware validation; untaken branches recorded as `SKIPPED`), and wave-based parallel execution (`maxConcurrency` default 4 with deterministic topological step/event ordering). `agent` nodes execute through the canonical `BoundedAgentRuntime`, and READY workflows are registerable as first-class tools callable by agents (workflow-as-tool with recursion depth limit). Shares the canonical workflow contract, tool registry, event/outbox envelope, and unit-of-work boundary. | Unit-, route-, and live-HTTP-tested in demo/memory mode (192 tests; loop/route/branch/fan-out-merge smoke over real HTTP); migration SQL verified on PostgreSQL 16 (PGlite WASM); Prisma adapter server path ready but `UNVERIFIED` until the next disposable-cluster run. |
...
5. Implement the remaining WORKFLOW ENGINE & VISUAL AUTOMATION DIRECTIVE items per the §56 compliance map: webhook/schedule idempotency (§48), public workflow interfaces (§34), durable scheduling and approval resumption (§32/§49), router/loop persisted decision records, and canvas UI actions for the Orcflo surface. Deterministic branching, routing, bounded loops, and parallel execution now exist in the Orcflo engine; selecting a validated external planner provider remains an Agent Runtime action.
