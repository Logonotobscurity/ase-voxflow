...
5. Implement the remaining WORKFLOW ENGINE & VISUAL AUTOMATION DIRECTIVE items per the §56 compliance map: public workflow interfaces (§34), durable scheduling and approval resumption (§32/§49), router/loop persisted decision records, and canvas UI actions for the Orcflo surface. Deterministic branching, routing, bounded loops, parallel execution, and run idempotency now exist in the Orcflo engine; selecting a validated external planner provider remains an Agent Runtime action.
