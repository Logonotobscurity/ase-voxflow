...
| 2026-08-16 | **Resolved multi-worker run claim/lease for the durable run worker (mirrors the outbox, Audit §3).** `OrcfloRun` carries `claimedBy`/`claimedUntil`; `claimBatch` atomically claims PENDING runs (or expired leases) for a worker — `FOR UPDATE SKIP LOCKED` in Prisma, a serialized lock in memory — and `releaseClaim` is ownership-conditional. `RunDispatcher` claims → executes (`executeRun` with its worker id) → releases on parked/terminal states; `executeRun` refuses runs claimed by another live worker. | Removes the single-worker limitation of the durable-execution increment: multiple run workers can drain PENDING runs without duplicating work, reclaiming after a crashed worker's lease expires. Lease renewal/heartbeat remains future work (the outbox shares this boundary). Evidence: `docs/verification/orcflo-engine-2026-08-16.md` §20; migration `20260817001000_orcflo_run_claim_lease` verified on PostgreSQL 16 (PGlite). |
