...
| Implement deterministic branch evaluation and connect `agent` workflow nodes. | Branch coverage, persisted decisions, bounded-runtime wiring, and observable evidence. | **Workflow Runtime Owner** | PARTIALLY RESOLVED (2026-08-16): `agent` workflow nodes now execute through the canonical `BoundedAgentRuntime` (see the AGENT ⇄ WORKFLOW bridge decision-log entry), and the workflow directive (§12–§14) is captured at `docs/WORKFLOW-ENGINE-DIRECTIVE.md`. Deterministic per-edge conditional selection, router nodes, and bounded loops remain UNRESOLVED. |
...
| Orcflo run engine | Runs with a replayable event stream, opt-in content-hashed step cache, metering, fail-closed model gateways (`ai_model` nodes), four trigger kinds, and blueprint instantiation into first-class workflows. `agent` nodes execute through the canonical `BoundedAgentRuntime`, and READY workflows are registerable as first-class tools callable by agents (workflow-as-tool with recursion depth limit). Shares the canonical workflow contract, tool registry, event/outbox envelope, and unit-of-work boundary. | Unit-, route-, and live-HTTP-tested in demo/memory mode; migration SQL verified on PostgreSQL 16 (PGlite WASM); Prisma adapter server path ready but `UNVERIFIED` until the next disposable-cluster run. |
...
5. Implement the remaining WORKFLOW ENGINE & VISUAL AUTOMATION DIRECTIVE items per the §56 compliance map: per-edge conditional selection, router nodes, bounded loops, workflow-tool idempotency, public workflow interfaces, and durable scheduling/approval resumption. Agent nodes already run through the canonical runtime with a bounded deterministic planner; selecting a validated external planner provider remains an Agent Runtime action.
