...
| ADR-019 | **Multi-worker run claim/lease:** PENDING runs are claimed by workers with `claimedBy`/`claimedUntil` (atomic `FOR UPDATE SKIP LOCKED` in Prisma, serialized lock in memory), reclaimed after lease expiry, and released when parked/terminal; `executeRun` refuses runs claimed by another live worker. Mirrors the outbox claim/lease protocol. | Accepted 2026-08-16 |
